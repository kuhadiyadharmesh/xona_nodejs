...
exports.user_update_profile = function (req, res)
{
    if(req.body.type == 2)
    {
        ...
        CommonFile.admin_auth(req.headers.token , (err , value)=>
        {
            ...
            User.findOneAndUpdate({_id: new Object(advertiser_id)}, {$set: data},{new: true}, function (err, user) 
                    {
                        if(err)
                        {
        ...
        CommonFile.check_authWithId(req.headers.token  , (err , user_id)=>
        {
            ...
            else
            {
              ...
              // delete data['referral_code']
                delete data['wallet_balance']
                delete data['_id']
                User.findOneAndUpdate({_id: new Object(user_id)}, {$set: data},{new: true}, function (err, user) 
                    {
                        if(err)
                        {
                          return  res.send(JSON.stringify({ status : 0 , msg : "something gone wrong." , err : err})); 
...
exports.admin_getall_user = function(req , res)
{
    CommonFile.admin_auth(req.headers.token, (err , value )=>
    {
        ...
        else
        {
            ...
            if(type == 0)
            {
                User.count({$or : [{'mobile_number': new RegExp(search_data, 'i')} , {'first_name': new RegExp(search_data, 'i')} 
                ,{'last_name': new RegExp(search_data, 'i')} ,{'myCode': new RegExp(search_data, 'i')}]},function(err,cou){if (err) {} else {t_data= cou;
                    User.find({$or : [{'mobile_number': new RegExp(search_data, 'i')}, {'first_name': new RegExp(search_data, 'i')} 
                    ,{'last_name': new RegExp(search_data, 'i')} ,{'myCode': new RegExp(search_data, 'i')}]},function(err,user){
                        if (err)
                       return res.send({ status : 0 , msg : "something gone wrong." , err : err}); 
                        else
                        {
            ...
            if(type == 1)
            {
                User.count({is_active : true , is_verify : true , is_admin_approve : 0 , $or : [{'mobile_number': new RegExp(search_data, 'i')}, {'first_name': new RegExp(search_data, 'i')} 
                ,{'last_name': new RegExp(search_data, 'i')} ,{'myCode': new RegExp(search_data, 'i')}]},function(err,cou){if (err) {} else {t_data= cou;
                    User.find({is_active : true , is_verify : true , is_admin_approve : 0 , $or : [{'mobile_number': new RegExp(search_data, 'i')}, {'first_name': new RegExp(search_data, 'i')} 
                    ,{'last_name': new RegExp(search_data, 'i')} ,{'myCode': new RegExp(search_data, 'i')}]},function(err,user){
                        if (err)
                       return res.send({ status : 0 , msg : "something gone wrong." , err : err}); 
                        else
            ...
            if(type == 2)
            {
                User.count({is_active : true , is_verify : true , is_admin_approve : 2 , $or : [{'mobile_number': new RegExp(search_data, 'i')}, {'first_name': new RegExp(search_data, 'i')} 
                ,{'last_name': new RegExp(search_data, 'i')} ,{'myCode': new RegExp(search_data, 'i')}]},function(err,cou){if (err) {} else {t_data= cou;
                    User.find({is_active : true , is_verify : true , is_admin_approve : 2 , $or : [{'mobile_number': new RegExp(search_data, 'i')}, {'first_name': new RegExp(search_data, 'i')} 
                    ,{'last_name': new RegExp(search_data, 'i')} ,{'myCode': new RegExp(search_data, 'i')}]},function(err,user){
                        if (err)
                      return  res.send({ status : 0 , msg : "something gone wrong." , err : err}); 
                        else
            ...
            if(type == 3)
            {
                User.count({is_active : true , is_verify : true , is_admin_approve : 1 ,  $or : [{'mobile_number': new RegExp(search_data, 'i')}, {'first_name': new RegExp(search_data, 'i')} 
                ,{'last_name': new RegExp(search_data, 'i')} ,{'myCode': new RegExp(search_data, 'i')}]},function(err,cou){if (err) {} else {t_data= cou;
                    User.find({is_active : true , is_verify : true , is_admin_approve : 1 ,  $or : [{'mobile_number': new RegExp(search_data, 'i')}, {'first_name': new RegExp(search_data, 'i')} 
                    ,{'last_name': new RegExp(search_data, 'i')} ,{'myCode': new RegExp(search_data, 'i')}]},function(err,user){
                        if (err)
                      return  res.send({ status : 0 , msg : "something gone wrong." , err : err}); 
                        else
