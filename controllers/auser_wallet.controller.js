...
exports.credit_balance = function(req , res)
{
    CommonFile.admin_auth_withData(req.headers.token , (err , value)=>{
        ...
        else
        {
            ...
            let c_type = 3 // 1 for online , 2 for refund from ad. , 3 for admin add
            let c_type_id = value._id
            let pay_from = "";
    ...
    //{"user_id":"5bfbbe52dfd6ee141037e8d8","charge_amount":15,"old_balance":10}
    ...
    if(req.body.is_admin  == 2)
    {
        ...
        CommonFile.admin_auth(req.headers.token ,(error , value) =>
        {
            ...
            else
            {
               ...
               if(type == 1)
               {
                Wallet.count({user_id : ObjectId(user_id)},function(err,cou){if (err) {res.send({status : 0 , msg :"Something went wrong."})} else 
                {
                   Wallet.aggregate([{ 
                    "$match" : {
                ...
                {
                    Wallet.count({user_id : ObjectId(user_id) , wallet_type:2 , details_type : 1},function(err,cou){if (err) {res.send({status : 0 , msg :"Something went wrong."})} else 
                    {
                        Wallet.aggregate([{ 
                            "$match" : {
                                "user_id" : ObjectId(user_id) ,
