...
const NuserSession = require('../../models/Session_model/nuser_session.model')
...
exports.user_send_otpv2 = function (req, res) {
    ...
    NUser.find( { $or : [{ mobile_number :  mobile_n},{ mobile_second :  mobile_n},{mobile_number : mobile_n} ]}, function(err, nuser){
        ...
        else
        {
            ...
            if(nuser.length == 0)
            {
                ...
                CommonFile.send_otp1(mobile_n,otp,(err , otp)=>
                {
                    if(err)
                    {
                        res.send({ status : 0 , msg : "something gone wrong." , err : err});
            ...
            else
            {
                ...
                if(nuserdata.mobile_number ==  mobile_n) //&& nuserdata.device_udid  == req.body.device_udid)
                {
                    ...
                    var otp = Math.floor(100000 + Math.random() * 900000);
                    CommonFile.send_otp1(nuserdata.mobile_number,otp,(err , val)=>
                    {
                        if(val)
                        {
                            NUser.findByIdAndUpdate(nuserdata.id, {$set: {OTP:otp}}, function (err, nuser)
...
/*
exports.check_ip = function(req , res)
{
    //const clientIp = publicIp.getClientIp(req); 
                ...
                if(nuser.is_verify == false)
                {
                    ...
                    if(req.body.referral_code == undefined || req.body.referral_code == "" )
                    {
                        ...
                        NUser.findByIdAndUpdate(nuser.id, {$set: {is_verify : true , token : new_token }}, function (err, nuser)
                        {
                                ...
                                {
                                    let nuser_Session = new NuserSession({
                                        user_id : nuser.id,
                                        user_token : new_token,
                                        login_ip : req.body.ip,
                                        login_lat : req.body.lat,
                                        login_long : req.body.longi,
                                        cdate : currentDate
                                    })
                                        nuser_Session.save(function(err , saveData){
                                            if(err)
                                            {
                                                res.send({ status : 0 , msg : "something gone wrong." , err : err}); 
                                            }
    ...
    CommonFile.admin_auth(req.headers.token , (err , value)=>
    {
        ...
        else
        {
           ...
           delete data['referral_code']
            delete data['point_balance']
            delete data['_id']
                    NUser.findByIdAndUpdate(req.body.enduser_id, {$set: data}, function (err, nuser) 
                    {
                        if (err) 
                       return res.send({ status : 0 , msg : "something gone wrong." , err : err});
                        else
    ...
    NCommonFile.nuser_check_authWithData(req.headers.token , (err , nuser1)=>{
        ...
        if (nuser1)
        {
           ...
           delete data['referral_code']
            delete data['point_balance']
            delete data['_id']
            NUser.findByIdAndUpdate(nuser1._id, {$set: data}, function (err, nuser) 
            {
                if (err) 
                res.send({ status : 0 , msg : "something gone wrong." , err : err});
                else
    ...
    CommonFile.admin_auth(req.headers.token , (err , value)=>
    {
        ...
        else
        {
            ...
            {//1 for not approved //2 for rejected by admin// 3 for approved & running
                NUser.count({$or : [{'mobile_number': new RegExp(mobile_number, 'i')} , {'first_name': new RegExp(mobile_number, 'i')} 
                ,{'last_name': new RegExp(mobile_number, 'i')} ,{'myCode': new RegExp(mobile_number, 'i')} ]},function(err,cou){if (err) {} else {t_data= cou;
                    NUser.find({$or : [{'mobile_number': new RegExp(mobile_number, 'i')} , {'first_name': new RegExp(mobile_number, 'i')} 
                    ,{'last_name': new RegExp(mobile_number, 'i')} ,{'myCode': new RegExp(mobile_number, 'i')} ]},function(err,nuser){
                        if (err)
                       return res.send({ status : 0 , msg : "something gone wrong." , err : err}); 
                        else
                        {
            ...
            if(type == 1)
            {
                NUser.count({is_active : true,is_verify: true , is_approve : 0 ,  $or : [{'mobile_number': new RegExp(mobile_number, 'i')} , {'first_name': new RegExp(mobile_number, 'i')} 
                ,{'last_name': new RegExp(mobile_number, 'i')} ,{'myCode': new RegExp(mobile_number, 'i')}]},function(err,cou){if (err) {} else {t_data= cou;
                    ...
                    NUser.aggregate([
                        ...
                        { 
                            "$match" : {
                                ...
                                "is_verify" : true ,
                                "is_active" : true , 
                                "$or" :[{"mobile_number":{"$regex": mobile_number,"$options":'i'}},{"first_name":{"$regex": mobile_number,"$options":'i'}},{"last_name":{"$regex": mobile_number,"$options":'i'}},{"myCode":{"$regex": mobile_number,"$options":'i'}}]
                            }
                        }, 
                        { 
                            "$sort" : {
            ...
            if(type == 2)
            {
                NUser.count({is_active : true,is_verify: true , is_approve : 2 , $or : [{'mobile_number': new RegExp(mobile_number, 'i')} , {'first_name': new RegExp(mobile_number, 'i')} 
                ,{'last_name': new RegExp(mobile_number, 'i')} ,{'myCode': new RegExp(mobile_number, 'i')}]},function(err,cou){if (err) {} else 
                {t_data= cou;
            ...
            if(type == 3)
            {
                NUser.count({is_active : true,is_verify: true , is_approve : 1 , $or : [{'mobile_number': new RegExp(mobile_number, 'i')} , {'first_name': new RegExp(mobile_number, 'i')} 
                ,{'last_name': new RegExp(mobile_number, 'i')} ,{'myCode': new RegExp(mobile_number, 'i')}]},function(err,cou){if (err) {} else {t_data= cou;
