        ...
        else
        {
            ...
            NUser.findOne({_id:user_id , is_approve: 0},function(err , nuserData)
            {
                if(nuserData)
                {
                        ...
                        NCommonFile.point_system(1,(err , pointdata)=>
                        {
                            ...
                            wallet.save(function(err , wallet){
                                ...
                                else
                                {
                                    ...
                                    if(nuserData.referral_code ==  undefined)
                                    {}
                                    else
                                    {
                                        ...
                                        console.log(1)
                                        Referral_Pending.findOneAndUpdate({user_id:nuserData._id , referral_code:nuserData.referral_code , is_paid : false },{$set : {is_paid : true}}, function(err , pending_referral){
                                            console.log(2)
                                            console.log(pending_referral)
                                            if(pending_referral)
                                            {
                                                NUser.findOne({myCode : nuserData.referral_code},function(err , touserData){
                                                    console.log(3)
                                                    ...
                                                    if(touserData)
                                                    {
                                                        ...
                                                        let touserwallet = new Wallet({
                                                            ...
                                                            advertise_type : -1 ,//  -1 for othes ,  ad type 1 for banner , 2 for half banner , 3 for full screen 
                                                            details_id: nuserData._id,//{type: Schema.Types.ObjectId,ref: 'AdMaster'},// credit (Online table_id , Refund (advertise id ) , debit (advertise id)
                                    ...
                                    NUser.findByIdAndUpdate({_id : user_id , is_approve : 0 , is_verify : true , is_active : true },{$set :{is_approve : 1 , myCode:code , point_balance : pointdata.points ,update_date:cdate}},function(err , nuser){
                                        if(err)
                                        {
                                           return res.send({status : 0,msg : "something gone wrong.",errm : err })
                                        }
                    ...
                    {
                        NUser.findByIdAndUpdate({_id : user_id , is_approve : 0 , is_verify : true , is_active : true },{$set :{is_approve : 2 , token : "Rejected-tok"+Math.floor(100000000 + Math.random() * 900000000) ,update_date:cdate}},function(err , nuser){
                            if(err)
                            {
                               return res.send({status : 0,msg : "something gone wrong.",errm : err })
                            }
...
exports.admin_getall_users = function(req , res)
{
    CommonFile.admin_auth(req.headers.token , (err , value)=>
    {
        ...
        else
        {
            ...
            if(type == 2)
            {
                NUser.count({is_active : true,is_verify: true , is_approve : 2 , $or : [{'mobile_number': new RegExp(mobile_number, 'i')}]},function(err,cou){if (err) {} else {t_data= cou;
