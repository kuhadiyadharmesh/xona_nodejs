...
exports.getAdvertise = function(req , res)
{
    NCommonFile.nuser_check_authWithData(req.headers.token , (err , nuserData) =>
    {
        ...
        else
        {
            ...
            {
                AdMaster.find({is_active : true ,is_approve : 1 , is_status : 1}, function(err , admaster){
                    if(err)
                    {
                    ...
                    else
                    {
                        ...
                        {
                           return res.send({status : 1 , msg : "no advertise found.", data : admaster})
                        }
                        else
                        {
                            let all_ad_ids = []
                            admaster.forEach(element =>
                                {
                                    all_ad_ids.push(element._id)
                                })
                                console.log(all_ad_ids)
                               
                            AdShow_History.find({nuser_id : ObjectId(nuserData._id) , is_status : 1 , advertise_id : {$in : all_ad_ids.map(ObjectID)}},function(err ,adshowhistory){
                                if(err)
                                {
                                    return res.send({status : 0 , msg : "contact xona support ",msg2 : "developer issues"})
                                }
                                else
                                {
            ...
            else
            {
                ...
                AdMaster.find({is_active : true , is_approve : 1 , is_status : 1 , $expr:{$gt:["$totaluser_for_ads", "$totaluser_visitor"]}},{ _id : 1},function(err , admasterids)
                {
                    ...
                    else
                    {
                        ...
                        var last_date=dateFormat(new Date(new Date().setDate(new Date().getDate()-1)), "yyyy-mm-dd");
                       // var day1=dateFormat(new Date(new Date().setDate(new Date().getDate()+1)), "yyyy-mm-dd");
                       console.log(dateFormat(new Date(), "yyyy-mm-ddTHH:MM:ssZ "))
                       console.log(dateFormat(new Date()))
                       console.log(new Date())
                       console.log({nuser_id : ObjectId(nuserData._id) , is_status : 1 ,"cdate" : { $lt: new Date(last_date+"T18:29:59Z").toISOString()}})
                        //,"cdate" : { $lt: new Date(day+"T00:00:00Z").toISOString(), $gte: new Date(day+"T23:59:59Z").toISOString()} 
                        // AdShow_History.find({nuser_id : ObjectId(nuserData._id) , is_status : 1 ,"cdate" : { $lt: new Date(day+"T00:00:00Z").toISOString(), $gte: new Date(day1+"T00:00:00Z").toISOString()}},{advertise_id : 1},function(err , adhistoryDatas)
                        //AdShow_History.find({nuser_id : ObjectId(nuserData._id) , is_status : 1 , is_team_paid : true ,"cdate" : { $lt: new Date(day+"T23:59:59Z").toISOString()}},{advertise_id : 1},function(err , adhistoryDatas){
                       
        
                       // old showen ads
                        AdShow_History.find({nuser_id : ObjectId(nuserData._id) , is_status : 1 ,"cdate" : { $lt: new Date(last_date+"T18:29:59Z").toISOString()}},{advertise_id : 1},function(err , adhistoryDatas_old){
                                ...
                                if(adhistoryDatas_old.length == 0)
                                {
                                   ...
                                   // let all_ids = []
                                    /*old_hisotry_id.forEach(element =>
                                        ...
                                        AdShow_History.find({nuser_id : ObjectId(nuserData._id) , is_status : 1 ,"cdate" : { $lt: new Date(current_date+"T18:29:59Z").toISOString(), $gte: new Date(last_date+"T18:29:59Z").toISOString()}},{advertise_id : 1},function(err , adhistoryDatas)
                                        {
                                            let all_ids = []
                                            old_hisotry_id.forEach(element =>
                                                {
                                    ...
                                    admasterids.forEach(element =>
                                        {
                                            ...
                                            adhistoryDatas_old.forEach(elemt_1=>{
                                                //console.log(elemt_1.advertise_id+'-----------'+element._id)
                                                if((elemt_1.advertise_id).equals(element._id))
                                                {
                                                    f_f = false 
                                                }
