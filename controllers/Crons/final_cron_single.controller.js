...
var logger = winston.createLogger({
    transports: [
      ...
      new winston.transports.File({ filename: 'test_20190214.log' })
    ]
  });
///////////////////////////////////////////////////////////----- Cron Setup ------///////////////////////////////
...
var cron_check = cron.schedule('01 00 18 * * *',()=>{
 ...
 var cron_target_count = cron.schedule('00 29 11 * * *', () =>{
    var day_today=dateFormat(new Date(new Date().setDate(new Date('2019-02-26').getDate())), "yyyy-mm-dd");
...
//var cron_advertise_live = cron.schedule('01 20 22 * * *',() =>{
    var cron_advertise_live = cron.schedule('01 00 05 * * *',() =>{
    make_live_ads()
})*/

var cron_temparary_point_credit = cron.schedule('10 35 11 * * *',() =>{
    var day_today=dateFormat(new Date(new Date().setDate(new Date('2019-02-26').getDate())), "yyyy-mm-dd");
   //var cron_temparary_point_credit = cron.schedule('01 30 19 * * *',() =>{
   // var day_today=dateFormat(new Date(new Date().setDate(new Date().getDate())), "yyyy-mm-dd");
    start_provide_downlinepoint_temp(day_today)
})
var cron_final_point_credit = cron.schedule('10 37 11 * * *',()=>{
    var day_today=dateFormat(new Date(new Date().setDate(new Date('2019-02-26').getDate())), "yyyy-mm-dd");
