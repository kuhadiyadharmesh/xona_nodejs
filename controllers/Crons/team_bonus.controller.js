...
let dev_db_url = 'mongodb://root:netdroidtech123@ds241375-a0.mlab.com:41375/xona';
        ...
        /*Cron_activity.findOne({record : day_i},function(err , record)
        {
            ...
            {//if(record)
                ...
                cron_activity.save(function(err , record_save)
                {
                    ...
                    {
                        logger.info('Total user :'+nUserData.length);
                        if(err)
                        {
                            console.log("err + "+err)
                            logger.error('error line NO 68 :');
                        }
                        else
                        {
                            AdMaster.find({is_active : true ,is_approve : 1 , is_status : 1},{ _id : 1}, function(err , admasterids){
					logger.error('admaser find :'+admasterids);
                                if(err)
                                {
                                    console.log("err + "+err)
                                    logger.error('error line NO 76 :');
                                }
                                else
                                {
                                    ...
                                    nUserData.forEach(useelement =>
                                        {
                                            ...
                                            // var day=dateFormat(new Date(new Date().setDate(new Date().getDate()-1)), "yyyy-mm-dd");
                                             ...
                                             var day_today=dateFormat(new Date(new Date().setDate(new Date().getDate())), "yyyy-mm-dd");
                                                ...
                                                Ad_show_history.find({nuser_id : ObjectID(useelement._id) , is_status : 1 ,"cdate" : { $lt: new Date(day+"T18:29:00Z")}},{advertise_id : 1},function(err , adhistoryDatas_old){
                                                 if(err)
                                                 {
                                                    logger.error('error line NO 95 : for User :-'+useelement._id);
                                                    logger.error('error line NO 95 :'+err);
                                                    //console.log("----")
                                                 }
                                                 else
                                                 {
                                                     ...
                                                     if(adhistoryDatas_old.length == 0)
                                                     {
                                                         ...
                                                         Ad_show_history.find({nuser_id : ObjectID(useelement._id) , is_status : 1 ,"cdate" : { $lt: new Date(day_today+"T18:29:59Z"), $gte: new Date(day+"T18:30:00Z")}},{advertise_id : 1},function(err , adhistoryDatas)
                                                             {
                                                                 let all_ids = []
                                                                     ...
                                                                     adshowData.forEach(element =>
                                                                         ...
                                                                         target_history.save(function(err , targ)
                                                                         {
                                                                             if(err)
                                                                             {
                                                                                 ...
                                                                                 logger.error('error line NO 169 : for User :-'+err);
                                                                             }
                                                                             else
                                                                             {
                                                                                logger.info('target History saved for User :-'+targ);
...
//var advertise_pushlive = cron.schedule('01 20 22 * * *',() =>
  {
    let vdate = new Date();
...
advertise_pushlive.start()
  ...
  //var c2 = cron.schedule('01 35 22 * * *',() =>
  {
      ...
      Target_History.find({achive_target :{$gte : 99} , target_date : day_i},function(err , target_achive_usersData)
      {
              ...
              NUserWallet.find({user_id : {$in : user_ids.map(ObjectID)},wallet_type : 1 , details_type : 2 , ispoint_temp : 1,"cdate" : { $lt: new Date(day_today+"T18:29:59Z"), $gte: new Date(day+"T18:30:00Z")}},function(err , wallet_data)
              {
                  if(err)
                  {
                  ...
                  else
                  {
                      ...
                      NUserWallet.updateMany({user_id : {$in : user_ids.map(ObjectID)},wallet_type : 1 , details_type : 2 , ispoint_temp : 1,"cdate" : { $lt: new Date(day_today+"T18:29:59Z"), $gte: new Date(day+"T18:30:00Z")}},{$set :{ispoint_temp : 2}},function(err , dataupdated)
                      {
                          if(err){}
                          else
                          {
                            ...
                            // Rejected for user
                            NUserWallet.updateMany({wallet_type : 1 , details_type : 2 , ispoint_temp : 1,"cdate" : { $lt: new Date(day_today+"T18:29:59Z"), $gte: new Date(day+"T18:30:00Z")}},{$set :{ispoint_temp : 3}},function(err , dataupdated1)
                            {
                                if(err){}
                                else
                                {
  ...
  var test_cron = cron.schedule('00 00 23 * * *',()=>{
      console.log('all job restart')
      console.log('all job restart')
      c1.stop()
      c2.stop()
      c3.stop()
      ctest.stop()
      console.log('all job start')
      console.log('all job start')
      c1.start()
      c2.start()
      c3.start()
